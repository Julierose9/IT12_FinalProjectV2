...
.filter-menu {
    ...
    .status-completed { background: #e8f5e8; color: #23b07a; }
    .status-cancelled { background: #fde8e8; color: #e05252; }
            ...
            <div class="filter-menu" id="filterMenu">
              <div class="filter-section mb-3">
                ...
                <div class="form-check">
                  <input class="form-check-input" type="radio" name="period" id="period-today">
                  <label class="form-check-label" for="period-today">Today</label>
                </div>
                <div class="form-check">
                  <input class="form-check-input" type="radio" name="period" id="period-week">
                  <label class="form-check-label" for="period-week">This Week</label>
                </div>
                <div class="form-check">
                  <input class="form-check-input" type="radio" name="period" id="period-month" checked>
                  <label class="form-check-label" for="period-month">This Month</label>
                </div>
              </div>
              <div class="filter-section">
                <div class="filter-section-title">Status</div>
                <div class="form-check">
                  <input class="form-check-input" type="checkbox" id="status-completed" checked>
                  <label class="form-check-label" for="status-completed">Completed</label>
                </div>
                <div class="form-check">
                  <input class="form-check-input" type="checkbox" id="status-cancelled" checked>
    ...
    <div class="card-body">
      <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <div class="d-flex gap-2">
          <a href="{{ route('admin.transaction') }}?export=csv" class="btn btn-success btn-sm">
            <i class="fas fa-file-csv me-1"></i> CSV
          </a>
          <a href="{{ route('admin.transaction') }}?export=pdf" class="btn btn-danger btn-sm">
      ...
      <div class="table-responsive">
        <table class="table table-hover align-middle">
          ...
          <tbody>
            ...
            <tr>
              <td><strong>#{{ $t->TransactionID ?? $t->id }}</strong></td>
              <td>
                <div style="font-weight:600">
                  {{ $t->cashier?->EmpFName ?? 'System' }} {{ $t->cashier?->EmpLName ?? '' }}
                </div>
              </td>
              <td>{{ $t->items_count ?? 0 }} items</td>
              <td><strong>₱{{ number_format($t->total_amount ?? 0, 2) }}</strong></td>
              <td><span class="text-primary">{{ $t->payment_method ?? 'Cash' }}</span></td>
              <td>
            ...
            @empty
            <tr>
              <td colspan="8" class="text-center py-5 text-muted">No transactions found</td>
            </tr>
            @endforelse
          </tbody>
        </table>
